...
function toDisplayValue(
  ...
  if (metric === 'public_network_transfer_bytes' || metric === 'private_network_transfer_bytes') {
...
export async function GET(request: Request) {
  ...
  for (const row of rows) {
    ...
    const raw = metricToSafeNumber(readMetricValue(row, metric as NeonUsageMetricName));
