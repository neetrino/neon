...
1. Copy `.env.example` → `.env` and set `DATABASE_URL`, `NEON_API_KEY`, `NEON_ORG_ID`, `CRON_SECRET` (and optional `DASHBOARD_PASSWORD` + `JWT_SECRET`).
...
4. Deploy to Vercel; add the same env vars. Cron is defined in `vercel.json` (02:00 UTC).
