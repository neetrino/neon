...
After `pnpm install`, Git hooks (Husky) run **lint-staged** on commit and **`pnpm run ci`** (ESLint + Vitest) before push. CI runs the same on GitHub. Details: `docs/QUALITY_AUTOMATION_PLAN.md`.

| Script                              | Purpose                     |
| ----------------------------------- | --------------------------- |
| `pnpm lint`                         | ESLint (full tree)          |
| `pnpm format` / `pnpm format:check` | Prettier                    |
| `pnpm test` / `pnpm test:ci`        | Vitest (watch / single run) |
| `pnpm run ci`                       | Lint + `test:ci`            |

Branch protection (`main`), secret scanning, and Dependabot are recommended for production repos.
