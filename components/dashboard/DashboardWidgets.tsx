...
export function SyncPanel({ runs, onSyncNow, syncingNow }: SyncPanelProps) {
  ...
  if (!last) {
    return (
      <div
        className="inline-flex items-center gap-2.5 rounded-xl border border-amber-200 bg-amber-50/80 px-2.5 py-1.5 text-xs text-amber-900"
        role="status"
      >
  ...
  return (
    <div
      className={`inline-flex max-w-full items-center gap-2.5 rounded-xl border px-2.5 py-1.5 text-xs ${statusTone}`}
      role="status"
    >
